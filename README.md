...
```php
Debug::dump($obData, $strFlag = false)
```
_Выводит в месте вызова содержимое переданного объёкта. Если $strFlag отличен от false отладочная информация будет видна всем, в противном случае только аккаунтам с административной привилегией. Метод является обёрткой для стандартной функции var_dump()._

...
```
_Является надстройкой над \Bitrix\Main\Diag\Debug::dumpToFile($arData, 'var name', '\_\_var.log'). Из контекста и debug_backtrace() формируется полезная информация, такая как путь до файла, номер строки, имя переменной и передаётся вторым аргументом в стандартный API._
